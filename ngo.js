document.addEventListener("DOMContentLoaded", () => {
    ...
    function renderNGOs() {
        ...
        ngosToShow.forEach(ngo => {
            ...
            const donateAction = hasWebsite ? `window.open('${ngo.website}/donate', '_blank')` : `window.open('https://wa.me/${ngo.whatsapp}', '_blank')`;
            const connectAction = hasWebsite ? `window.open('${ngo.website}/contact', '_blank')` : `window.open('mailto:${ngo.email}', '_blank')`;

            ngoGrid.innerHTML += `
            <div class="ngo-card">
                <div class="ngo-card-top">
                    <img class="ngo-img" src="${ngo.image}" alt="${ngo.name}">
                    <div class="ngo-info">
                        <h3>${ngo.name}</h3>
                        <p>${ngo.location}</p>
                        <div class="ngo-icons">
                            <i class="fa-solid fa-envelope" onclick="window.open('mailto:${ngo.email}', '_blank')" title="Email"></i>
                            <i class="fa-solid fa-phone" onclick="window.open('tel:${ngo.phone}', '_blank')" title="Call"></i>
                            <i class="fa-brands fa-whatsapp" onclick="window.open('https://wa.me/${ngo.whatsapp}', '_blank')" title="WhatsApp"></i>
                        </div>
                    </div>
                </div>
                <p class="ngo-tag">${ngo.tag}</p>
                <p class="ngo-desc">${ngo.desc}</p>
                <div class="ngo-card-actions">
                    <button class="ngo-btn ngo-donate-btn" onclick="${donateAction}">Donate</button>
                    <button class="ngo-btn ngo-connect-btn" onclick="${connectAction}">Connect</button>
                </div>
            </div>
            `;
        });
    ...
    regForm.addEventListener("submit", (e) => {
        ...
        if (file) {
            ...
            reader.onload = function(e) {
                ...
                finishRegistration(newNgo);
            };
            reader.readAsDataURL(file);
        } else {
            newNgo.image = "https://files.catbox.moe/aag4y5.jpg"; // Default placeholder
            finishRegistration(newNgo);
        }
    });

    function finishRegistration(newNgo) {
        ngos.unshift(newNgo); // Add to top
        populateDropdowns();
        applyFilters();
        regForm.reset();
        fileChosenText.textContent = "No file chosen";
